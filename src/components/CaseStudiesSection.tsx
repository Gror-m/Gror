...
const caseStudies = [
  {
    ...
    details: "Built a high-end property launch funnel with Google Ads for real estate, premium landing pages, and real estate lead generation tracking.",
    badge: "Residential",
  },
  {
    title: "Commercial Property Campaign",
    highlight: "₹12Cr+ Revenue Generated • 5.6X ROI • 42% Increase In Site Visits",
    details: "Scaled commercial property visibility using data-driven Meta Ads for builders, audience retargeting, and high-converting digital assets.",
    badge: "Commercial",
  },
  {
    title: "Builder Brand Growth Campaign",
    highlight: "3X Faster Lead Conversion • 8,000+ Leads Generated • 92% Automation Efficiency",
    details: "Enabled builder lead generation at scale with WhatsApp automation for real estate, CRM workflows, and performance marketing services.",
    badge: "Builder",
  },
];
  ...
  return (
    <section id="case-studies" className="mx-auto max-w-[1440px] px-6 py-16 md:px-12 lg:px-16">
      <div className="mb-10">
        ...
        <h2 className="mt-4 text-3xl font-black text-slate-950 sm:text-4xl">Real Results. Real Growth.</h2>
      </div>
      <div className="grid gap-6 lg:grid-cols-3">
        {caseStudies.map((item, index) => (
          <motion.article key={item.title} initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.8, delay: index * 0.12 }} className="rounded-[32px] border border-white/80 bg-white/80 p-8 shadow-[0_24px_70px_rgba(91,33,182,0.08)] backdrop-blur-xl hover:-translate-y-1 hover:shadow-[0_35px_90px_rgba(91,33,182,0.12)]">
            ...
            <p className="mt-4 text-sm leading-7 text-slate-500">{item.details}</p>
          </motion.article>
        ))}
      </div>
    </section>
